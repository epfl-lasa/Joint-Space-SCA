...
{
	int n_threads = 1;
    if (argc==4) n_threads = stoi(argv[3]);
    int dataset_size = stoi(argv[2]);
	string fp_idx = argv[1];
    ArrayXi local_size(n_threads);
    local_size = 0 * local_size;
    while (local_size.sum()<dataset_size)
    {
        local_size = local_size+200;
    }
    cout << local_size.transpose() << endl;
    cout << "init ok" << endl;
    vector<vector<ArrayXd>> data;
    data.resize(n_threads);
    ...
    #pragma omp parallel num_threads(n_threads)
    {
		...
		int n_col = 0;
		int n_noncol = 0;
        int dataset_size_thread = local_size[p_id];
		float d_threshold_close = 0.05; //in meters, if >100, then no distance queries at all.
		float d_threshold_collide = 0.001; //in meters, was 0.03 for candidacy proposal
		...
		int n_distanced = dataset_size_thread/2*dist_fract;
		int n_undistanced = dataset_size_thread/2  - n_distanced;

		bool ignore_stable = true;
		...
		switch(stoi(argv[1])) 
		{
			...
			case 11: 	rand_joints.resize(7); 		
						...
						break;	
			//plot given posture (uncomment first line in cycle "k+=1")
		...
		while(k<dataset_size_thread)
		{
			...
			if (ignore_stable || stable_flag){
				...
				if(collision_flag && n_col < dataset_size_thread/2)
				{	
					line[0] = 1;
					data[p_id].push_back(line);
					...
					if(k % 100 == 0) 
						cout << "Thread " << p_id << ": Total points: " << k <<", Collided: " << 1.0*n_col/k << endl;
				}

				if(!collision_flag && n_noncol < dataset_size_thread/2)
				{
					if(n_distanced == 0 && n_undistanced > 0.5*(dataset_size_thread/2 - dataset_size_thread/2*dist_fract)) //if we have collected all needed "close-to-collisions" and have no "far from collision"
					{
						dist_flag = true;
					...
					if(dist_flag && n_distanced > 0)
					{
						...
						k = k+1;
						if(k % 100 == 0) 
							cout << "Thread " << p_id << ": Total points: " << k <<", Collided: " << 1.0*n_col/k << "; " << n_distanced << " " << n_undistanced << endl;
    ...
    {
        res_size+=data[i].size();
        //cout << data[i][10].transpose() << endl;
    }

	string bin_fname = "../data/data_"+fp_idx+"_"+to_string(res_size/1000)+"k_"+to_string(t_start)+".bin";
	auto file = fstream(bin_fname, ios::out | ios::binary);
    file.write((char*) &res_size, sizeof(int));	
	int lsize = data[0][0].size();
	file.write((char*) &lsize, sizeof(int));	
    cout << "Writing .bin file!" << endl;
    for (int i = 0; i < n_threads; i++)
    {
	...
	// if(status){
	// 	string txt_fname = "../data/data_"+fp_idx+"_"+to_string(dataset_size/1000)+"k_"+to_string(t_start)+".txt";
